...
void* foo(void* p) {
    ...
    printf("Calling willAim in new thread");
    int i;
    while (1)
    {
        for (i = 8; i <= 190; i++)
    ...
    int benchmark, int benchmark_layers)
{
    ...
    //////////////////////// MY CODE - starting /////////////////////
    pthread_t newThread; // New thread for listening keyboard

    pthread_create(&newThread, NULL, foo, (void*)&newThread);

    //int* ptr;

    // Wait for foo() and retrieve value in ptr;
    //pthread_join(newThread, (void**)&ptr);
    ...
    while (1) {
        ++count;
        {
            ...
            ///////////////////////////////////////////////////////// MY CODE - starting /////////////////////////////////
            int postphoneNextAimInput = 0;
            int fired = 0;
            for (i = 0; i < local_nboxes; ++i)  //iterate all detected objects (max 80 in pre-trained file)
            {
                if (local_dets[i].prob[0] > demo_thresh)   //if detected object is PERSON 
                {
                    float x = local_dets[i].bbox.x * 1920.0;
                    float y = local_dets[i].bbox.y * 1080.0;
                    // eðer full screen stream alýnmayacak ise, gereken kaydýrma miktarý x ve y'ya baðlý parabolik fonksiyonlardan hesaplanýp x ve y 'ye eklenmeli

                    float w = local_dets->bbox.h;
                    float h = local_dets->bbox.w;
                    /*printf("x: %.3f,  y: %.3f", local_dets->bbox.x, local_dets->bbox.y);
                    printf("obje  x: %d,   y: %d \n", x, y);*/
                    float modifier = 3.0 / 0.25;

                    /*if (GetCursorPos(&p))
                    {
                        printf("mouse  x: %d,   y: %d \n", p.x, p.y);
                    }
                    //MOUSEEVENTF_ABSOLUTE   65535 , 32768-----  11800, 6640
                    //mouse_event(MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE, 32768, 32768, 0, 0);*/

                    //Fire when 'f' is pressed on keyboard
                    if (lastPressedKey == 70)   // 70 For key 'F'  
                    {
                        printf("\n                                                          Shooting");
                        lastPressedKey = -1;
                        fired = 1;
                        mouse_event(MOUSEEVENTF_MOVE, modifier * (x - 960.0), modifier * (y - 540.0), 0, 0);
                        mouse_event(MOUSEEVENTF_LEFTDOWN, 0, 0, 0, 0);
                        mouse_event(MOUSEEVENTF_LEFTUP, 0, 0, 0, 0);
                        // 4:3 rate     8850, 6637
                        // mouse_event(MOUSEEVENTF_MOVE | MOUSEEVENTF_ABSOLUTE, 32768, 32768, 0, 0);
                        //mouse_event(MOUSEEVENTF_MOVE, x - 4425, y - 3319 , 0, 0);
                        // break;
